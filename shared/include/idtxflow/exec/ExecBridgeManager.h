 ...
 * @brief The openUSD execution / compute model (openExec) allows to compute attributes of a specific schema (HasAPI or
 * IsA) based on other attributes of the same or other prims. This model uses some sort of pull mechanism that requires
 * to trigger computation and then requesting the computation results.
 *
...
enum class ExecBridgeLifecycle
{
    Created,
    Active,
    Stopping,
...
class ExecBridge
{
  public:
    ...
    ExecBridgeLifecycle GetLifecycle() const
    {
        return lifecycle_.load(std::memory_order_acquire);
    }
    bool Activate()
    {
        ExecBridgeLifecycle expected = ExecBridgeLifecycle::Created;
        return lifecycle_.compare_exchange_strong(expected, ExecBridgeLifecycle::Active, std::memory_order_acq_rel);
    }
    void BeginStopping()
    {
        ExecBridgeLifecycle state = GetLifecycle();
        while (state == ExecBridgeLifecycle::Created || state == ExecBridgeLifecycle::Active)
        {
            if (lifecycle_.compare_exchange_weak(state, ExecBridgeLifecycle::Stopping, std::memory_order_acq_rel))
                return;
        }
    }
    void FinishStopping()
    {
        {
            ...
            pending_update_order_.clear();
        }
        {
            std::lock_guard<std::mutex> lock(handler_mutex_);
            result_handlers_.clear();
        }
        lifecycle_.store(ExecBridgeLifecycle::Stopped, std::memory_order_release);
    }

    bool EnqueueAttributeUpdate(StageAttributeUpdate update)
    {
        if (GetLifecycle() != ExecBridgeLifecycle::Active || update.prim_path.IsEmpty() ||
            update.attribute_name.IsEmpty() || update.value.IsEmpty())
        {
            rejected_updates_.fetch_add(1);
            return false;
        }
        const std::string key = update.prim_path.GetString() + "." + update.attribute_name.GetString();
        std::lock_guard<std::mutex> lock(queue_mutex_);
        if (GetLifecycle() != ExecBridgeLifecycle::Active)
        {
            rejected_updates_.fetch_add(1);
            return false;
        }
        ...
        if (it != pending_updates_.end())
        {
            ...
            coalesced_updates_.fetch_add(1);
            return true;
        }
    ...
    int RegisterAttributeWithConnection(const pxr::UsdAttribute& attribute)
    {
        ...
        if (!attribute.GetConnections(&connections) || connections.empty()) return -1;
        ...
        if (primPath.IsEmpty() || propName.empty()) return -1;
        ...
        if (!sourcePrim.IsValid())
        {
            IDTX_LOG(IDTX_ERROR, "RegisterAttributeWithConnection: source prim '{}' not found for connection on '{}'",
                     primPath.GetText(), attribute.GetPath().GetText());
            return -1;
        }
        ...
        IDTX_LOG(IDTX_DEBUG, "AddValueKeyFromConnection: resolved connection '{}' -> prim '{}', computation '{}'",
                 attribute.GetPath().GetText(), primPath.GetText(), propName);

        return index;
    }

    /// Return the number of value keys registered.
    std::size_t GetValueKeyCount() const
    {
        return exec_value_keys_.size();
    }
    ...
    void BuildRequest()
    {
        ...
        auto invalidation_callback = [&](const pxr::ExecRequestIndexSet& indices,
                                         const pxr::EfTimeInterval& /* timeInterval*/) {
        ...
        exec_request_ = std::make_unique<pxr::ExecUsdRequest>(
            exec_system_->BuildRequest(std::move(exec_value_keys_), invalidation_callback));

        // finish compute request preparation for execution. This might be a heavy task, as this will "compile" the
        // compute tree for performance optimized execution
    ...
    void RegisterComputeResultHandler(const pxr::SdfPath& primPath, IExecBridgeHandler* handler)
    {
        ...
        if (std::find(handlers.begin(), handlers.end(), handler) == handlers.end()) handlers.push_back(handler);
    }
    ...
    void ComputeAndDispatch()
    {
        ...
        if (!exec_request_ || !exec_request_->IsValid()) return;
        ...
        // This will always publish an entry in the CacheView for all value keys passed to BuildRequest. However, the
        // ones that did not require recomputation, where provided from the cache. We could use the invalidation
        // callback to collect all the indices that has been really recalculated.
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            //     metadata.attributePath.GetText(), metadata.computationName.GetText(),
            //     pxr::TfStringify(computedValue).c_str());

            ExecComputeResult result = {metadata.attributePath.GetPrimPath(), metadata.attributePath.GetName(),
                                        metadata.computationName, computedValue, static_cast<int>(i)};
  ...
  private:
    void ApplyPendingUpdates()
    {
        ...
        {
            std::lock_guard<std::mutex> lock(queue_mutex_);
            for (const auto& key: pending_update_order_)
            {
                auto it = pending_updates_.find(key);
                if (it != pending_updates_.end()) updates.push_back(std::move(it->second));
            }
            pending_updates_.clear();
            pending_update_order_.clear();
        }
        for (const auto& update: updates)
        {
            ...
            const bool compatible =
                attribute && ((type == pxr::TfToken("string") && update.value.IsHolding<std::string>()) ||
                              (type == pxr::TfToken("float") && update.value.IsHolding<float>()) ||
                              (type == pxr::TfToken("double") && update.value.IsHolding<double>()));
            if (!compatible || !attribute.Set(update.value))
            {
                failed_updates_.fetch_add(1);
                continue;
            }
  ...
  protected:
    ...
    std::atomic<uint64_t> accepted_updates_{0}, coalesced_updates_{0}, rejected_updates_{0}, applied_updates_{0},
        failed_updates_{0};
...
class ExecBridgeManager
{
  public:
    ...
    void ActivateBridge(std::shared_ptr<ExecBridge> bridge)
    {
        ...
        if (std::find(active_bridges_.begin(), active_bridges_.end(), bridge) == active_bridges_.end())
            active_bridges_.push_back(std::move(bridge));
    }
    ...
    void DeactivateBridge(std::shared_ptr<ExecBridge> bridge)
    {
        ...
        std::lock_guard<std::mutex> lock(activation_mutex_);
        active_bridges_.erase(std::remove(active_bridges_.begin(), active_bridges_.end(), bridge),
                              active_bridges_.end());
