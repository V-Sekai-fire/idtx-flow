  ...
  public:
    ...
    static pxr::ArResolvedPath ResolvePath(const pxr::SdfAssetPath& assetPath, const pxr::UsdStageWeakPtr& stage)
    {
        ...
        std::string identifier =
            !resolved.empty()
                ? resolved
                : resolver.CreateIdentifier(assetPath.GetAssetPath(), stage->GetRootLayer()->GetResolvedPath());
        if (identifier.empty())
        {
    ...
    static std::shared_ptr<pxr::ArAsset> LoadAssetWithResolver(const pxr::SdfAssetPath& assetPath,
    ...
    static std::optional<typename Types::Texture> LoadTextureWithResolver(
        const pxr::SdfAssetPath& assetPath, const pxr::UsdStageWeakPtr& stage, const TexturePurpose purpose,
        idtxflow::cache::UsdResourceCache<TargetEngine>* resourceCache = nullptr)
    {
        // first check if we have already cached the texture for this asset file
        if (resourceCache)
        {
            if (auto cachedTexture = resourceCache->GetCachedTexture(assetPath.GetAssetPath());
                cachedTexture.has_value())
            {
