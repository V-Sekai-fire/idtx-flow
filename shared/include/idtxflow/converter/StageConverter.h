 ...
 *
 * This is the main entry point for the conversion of a USD stage and is responsible to coordinate the conversion of all
 * contained prims and assets into game engine specific entities and data structures. The actual conversion of the
 * individual prims and assets of the most common prim types is handled in this calss, while the engine specific
...
static inline std::string uuid_v4()
{
    ...
    for (auto& x: b)
        x = static_cast<unsigned char>(dist(rd));
...
template <typename TargetEngine>
    requires idtxflow::types::ValidTargetEngine<TargetEngine>
...
class UsdStageConverter
{
  ...
  public:
     ...
     */
    UsdStageConverter(typename Types::OwningEntity* inOwningEntity = nullptr,
                      cache::UsdResourceCache<TargetEngine>* inResourceCache = nullptr)
    {
     ...
     */
    [[nodiscard]]
    StageConversionResult<TargetEngine> Convert(const pxr::UsdStageRefPtr& stage,
                                                typename Types::ConvertedEntity* rootEntity = nullptr)
    {
        Stage = stage;
        ...
        std::vector<typename Types::ConvertedEntity*> convertedEntities =
            ConvertPrims(stage, stage->TraverseAll(), rootEntity);
        ...
        return {StageHandle(Stage), ConvertStagePostProcess(convertedEntities)};
    }
  ...
  protected:
     ...
     *         method has to be specialized to implement this.
     */
    std::vector<typename Types::ConvertedEntity*> ConvertPrims(const pxr::UsdStageRefPtr& stage,
                                                               const pxr::UsdPrimRange& primRange,
                                                               typename Types::ConvertedEntity* rootEntity = nullptr)
    {
        ...
        // Prims we need to maintain a mapping between the converted prim and the original to retrieve the correct
        // parent the mapping stores the original prims path within the stage and a reference/pointer to the converted
        // version
        std::map<pxr::SdfPath, typename Types::ConvertedEntity*> primToConvertedMap;

        // this lamda looks up the converted parent of a given Prim. As some Prims are only "structural" like
        // 'UsdScope', the direct parent might not be converted. Thus we recursively looking up the parents until we
        // have found one
        std::function<typename Types::ConvertedEntity*(const pxr::UsdPrim&)> getConvertedParent =
            [&](const pxr::UsdPrim& prim) -> typename Types::ConvertedEntity* {
    ...
    typename Types::ConvertedEntity* ConvertPrim(const pxr::UsdPrim& usdPrim, bool& outPruneChildren)
    {
        ...
        if (usdPrim.HasAuthoredPayloads() && !usdPrim.IsLoaded())
        {
            ...
            for (const pxr::PcpNodeRef& node: primIndex.GetNodeRange())
            {
                ...
                for (const pxr::SdfLayerRefPtr& layer: layerStack->GetLayers())
                {
                    ...
                    {
                        payloadUri =
                            pxr::ArGetResolver().CreateIdentifier(payload.GetAssetPath(), layer->GetResolvedPath());
            ...
            if (usdPrim.IsA<pxr::UsdGeomXform>())
            {
                ...
                bool resets;
                if (!xformable.GetLocalTransformation(&transform, &resets)) transform.SetIdentity();
            }
            else
            {
            ...
            // when converting the payload prim as seperate usd stage from it's uri we will loose the authored
            // properties or overrides in this very prim spec. Thus we will extract this prim spec into an anonymous
            // layer and pass this one to the conversation method as well to be applied when loading the stage. This
            // would compose the layer data into the stage and thus create the expected state
            ...
            if (payloadPrimPath.IsEmpty())
            {
                ...
                if (payloadLayer)
                {
                    if (payloadLayer->HasDefaultPrim())
                        targetPath = pxr::SdfPath::AbsoluteRootPath().AppendChild(payloadLayer->GetDefaultPrim());
                    else
            ...
            if (sessionPrimSpec && rootPrimSpec)
            {
                ...
                pxr::SdfCopySpec(composedLayer, // composerStage->GetRootLayer(),
                                 usdPrim.GetPath(), overrideLayer, targetPath);
            }
            else if (sessionPrimSpec)
            {
                // there is only the session spec for this prim - copy it
                pxr::SdfCopySpec(usdPrim.GetStage()->GetSessionLayer(), usdPrim.GetPath(), overrideLayer, targetPath);
            }
            else
            {
                // there is only the root spec for this prim - copy it
                pxr::SdfCopySpec(usdPrim.GetStage()->GetRootLayer(), usdPrim.GetPath(), overrideLayer, targetPath);
            }
            ...
            // we need to create an identifier for the layer with the path to the stage the layer shall provide a
            // stronger opinion for. Only then relative resource paths will be properly resolved. However, the
            // identifier is required to be unique when the same referred payload is used. Thus extract the unique part
            // of the anonymous id the layer is created with and use it as "filename" part of the layer identifyer.
            std::string layerPath =
                std::filesystem::path(usdPrim.GetStage()->GetRootLayer()->GetRealPath()).parent_path().string();
            std::string lp;
            ...
            std::string layer_identifier = pxr::SdfLayer::CreateIdentifier(
                layerPath + "/" + helper::uuid_v4() + ".usda", pxr::SdfLayer::FileFormatArguments{});
        ...
        else if (usdPrim.IsA<pxr::UsdGeomXform>() && usdPrim.HasAPI<pxr::IDTXInteractionAPI>())
        {
            ...
            convertedEntity = ConvertCollisionRoot(TypeConverter::toTransform(matrix), color, identifier.GetString(),
        ...
        else if (usdPrim.IsA<pxr::UsdGeomXform>())
        {
            ...
            std::optional<AnimationDescription<TargetEngine>> xFormAnimation =
                animationConverter.Convert(usdXform, StageTimecodesPerSec);
                   ...
                   pxr::UsdSkelBindingAPI(usdPrim).GetJointWeightsPrimvar().HasValue()))
        {
            ...
            if (IsPseudoinstance(usdPrim, &usdPseudioInstancePrototypePath))
            {
                ...
                convertedEntity = ConvertGprimPseudoInstance(TypeConverter::toTransform(matrix), usdGprim,
                                                             usdPseudioInstancePrototypePath);
            }
            else
            {
        ...
        else if (usdPrim.IsA<pxr::UsdSkelRoot>())
        {
            ...
            for (pxr::UsdPrim child: pxr::UsdPrimRange(usdPrim))
            {
                if (child.IsA<pxr::UsdSkelSkeleton>())
                {
                    ...
                    SkeletonDescription<TargetEngine> skeletonDescription =
                        skeletonConverter.Convert(usdSkelRoot, usdSkelSkeleton);

                    UsdAnimationConverter<TargetEngine> animationConverter;
                    std::optional<AnimationDescription<TargetEngine>> skeletonAnimation =
                        animationConverter.Convert(usdSkelRoot, usdSkelSkeleton, StageTimecodesPerSec);

                    convertedEntity =
                        ConvertSkeleton(TypeConverter::toTransform(matrix), skeletonAnimation, skeletonDescription);
  ...
  public:
    std::optional<typename Types::Material> ConvertMaterial(const pxr::UsdShadeMaterial& usdMaterial)
    {
        ...
        std::optional<types::MaterialDescription<typename UsdMaterialConverter<TargetEngine>::TextureType>>
            materialDescription = materialConverter.Convert(usdMaterial);
  ...
  protected:
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertPrimWithPayload(const pxr::UsdPrim& usdPrim, const std::string& payloadUri,
     ...
     */
    typename Types::ConvertedEntity* ConvertXform(const typename Types::Transform& transform,
    ...
    typename Types::ConvertedEntity* ConvertGprim(const pxr::UsdGeomGprim& usdGprim)
    {
        ...
        UsdAnimationConverter<TargetEngine> animationConverter;
        std::optional<AnimationDescription<TargetEngine>> gPrimAnimation =
            animationConverter.Convert(usdGprim, StageTimecodesPerSec);
        ...
        if (usdPrim.HasAPI<pxr::IDTXCollisionAPI>())
        {
            ...
            {
                main_axis = pxr::GfVec3f::XAxis();
            }
            else if (axis == pxr::UsdGeomTokens->y)
            {
                main_axis = pxr::GfVec3f::YAxis();
            }
            else if (axis == pxr::UsdGeomTokens->z)
            {
                main_axis = pxr::GfVec3f::ZAxis();
            }
        ...
        else if (usdPrim.IsA<pxr::UsdGeomCube>())
        {
            ...
            return ConvertCube(TypeConverter::toTransform(matrix), gPrimAnimation, material, cubeSize, displayColors,
                               colorInterpolation);
        }
        else if (usdPrim.IsA<pxr::UsdGeomCone>())
        {
            ...
            return ConvertCone(TypeConverter::toTransform(matrix, axis), gPrimAnimation, material, coneRadius,
                               coneHeight, displayColors, colorInterpolation);
        }
        else if (usdPrim.IsA<pxr::UsdGeomCylinder>())
        {
            ...
            return ConvertCylinder(TypeConverter::toTransform(matrix, axis), gPrimAnimation, material, cylinderRadius,
                                   cylinderHeight, displayColors, colorInterpolation);
        }
        else if (usdPrim.IsA<pxr::UsdGeomSphere>())
        {
            ...
            return ConvertSphere(TypeConverter::toTransform(matrix), gPrimAnimation, material, sphereRadius,
                                 displayColors, colorInterpolation);
        }
        else if (usdPrim.IsA<pxr::UsdGeomMesh>())
        {
            pxr::UsdGeomMesh usdMesh(usdPrim);
            std::vector<MeshDescription<typename UsdMeshConverter<TargetEngine>::MeshDataType>> meshDescriptions =
                meshConverter.Convert(usdMesh);

            return ConvertMesh(TypeConverter::toTransform(matrix), gPrimAnimation, meshDescriptions, displayColors,
                               colorInterpolation);
        }
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertGprimPseudoInstance(const typename Types::Transform& transform,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertCube(const typename Types::Transform& transform,
                                                 const std::optional<AnimationDescription<TargetEngine>>& animation,
                                                 const std::optional<typename Types::Material>& material,
                                                 float cubeSize, const pxr::VtArray<class pxr::GfVec4f>& displayColors,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertCylinder(const typename Types::Transform& transform,
                                                     const std::optional<AnimationDescription<TargetEngine>>& animation,
                                                     const std::optional<typename Types::Material>& material,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertCone(const typename Types::Transform& transform,
                                                 const std::optional<AnimationDescription<TargetEngine>>& animation,
                                                 const std::optional<typename Types::Material>& material,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertSphere(const typename Types::Transform& transform,
                                                   const std::optional<AnimationDescription<TargetEngine>>& animation,
                                                   const std::optional<typename Types::Material>& material,
                                                   float sphereRadius,
                                                   const pxr::VtArray<class pxr::GfVec4f>& displayColors,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertCollisionRoot(const typename Types::Transform& transform,
                                                          const pxr::GfVec3f highlightColor,
                                                          const std::string identifier, const bool enabled,
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertCollision(const typename Types::Transform& transform,
                                                      const pxr::TfToken shape, const pxr::VtArray<pxr::TfToken> types,
                                                      const pxr::GfVec3f axis, const double height,
                                                      const double radius);
    ...
    typename Types::ConvertedEntity* ConvertMesh(
        const typename Types::Transform& transform, const std::optional<AnimationDescription<TargetEngine>>& animation,
        const std::vector<MeshDescription<typename UsdMeshConverter<TargetEngine>::MeshDataType>>& meshDescriptions,
        const pxr::VtArray<class pxr::GfVec4f>& displayColors, const class pxr::TfToken& colorInterpolation);
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertSkeleton(const typename Types::Transform& transform,
                                                     const std::optional<AnimationDescription<TargetEngine>>& animation,
    ...
    bool IsPseudoinstance(const pxr::UsdPrim& usdPrim, pxr::SdfPath* prototypePath)
    {
        ...
        if (spec_attributes.has(pxr::UsdGeomTokens->points) || spec_attributes.has(pxr::UsdGeomTokens->normals) ||
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertPrimPostProcessDefault(const pxr::UsdPrim& usdPrim,
                                                                   ...
                                                                   typename Types::ConvertedEntity* convertedParentPrim)
    {
        ...
        {
            if (attribute.HasAuthoredConnections() && bridge->RegisterAttributeWithConnection(attribute) >= 0)
            {
        ...
        if (registeredComputeAttribute)
        {
            ...
            {
                IDTX_LOG(IDTX_WARN,
                         "Prim '{}' has connected compute attributes, but its converted entity does not expose "
                         "IExecBridgeHandler",
        ...
        if (IPrimConverter<TargetEngine>* converter = registry.Get(primTypeName))
        {
            ...
            convertedPrim =
                Types::ResolveConvertedEntity(converter->PostProcess(usdPrim, convertedHandle, parentHandle));
        }
     ...
     * @return
     */
    typename Types::ConvertedEntity* ConvertPrimPostProcess(const pxr::UsdPrim& usdPrim,
                                                            typename Types::ConvertedEntity* convertedPrim,
                                                            typename Types::ConvertedEntity* convertedParentPrim);
