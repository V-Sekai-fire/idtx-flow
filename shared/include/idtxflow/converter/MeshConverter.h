 ...
 *
 * Skeletal Prims are handled separately in the UsdSkeletalMeshConverter, which extracts the skinning data and skeleton
 * bindings. The actual mesh data is extracted in this converter and can be used by the skeletal mesh converter to
 * create the final skeletal mesh asset.
 */
#include <optional>
#include <vector>
...
namespace converter
{
template <typename MeshDataType> struct MeshDescription
{
...
class UsdMeshConverter
{
  public:
    ...
    {
        if (!usdMesh.GetPointsAttr().Get(&points)) return {}; // a mesh w/o points does not make any sense
        ...
        // To reduce this complexity we stick to the common naming convention to have the UV's authored as "st" or
        // "st0".
        ...
        // as the USD mesh may contain subsets we need to create the subset meshes first and finish of with the
        // remainder. while the USD subsets does only authore an index list of faces/edges/points into that refer to the
        // original mesh we need to create self-containing complete meshes for the target engine containing all vertices
        // and face indicies. we will store all subset faces that has been considered already. Only those faces that
        // have not been considered at all will then be converted in a final step. If a mesh does not contain any
        // subsets, this will lead to converting the whole mesh in the final step, as expected.
        ...
        for (const pxr::UsdGeomSubset& subset: pxr::UsdGeomSubset::GetAllGeomSubsets(usdMesh))
        {
            ...
            // push the subset mesh to the mesh data list. It's up to the engine to process the different meshes and
            // create the respective data in the engine format
    ...
    std::vector<MeshDescription<MeshDataType>> ConvertSkinnedMesh(
        const pxr::UsdSkelSkinningQuery& skinningQuery, const pxr::UsdSkelSkeletonQuery& skelQuery,
    ...
    pxr::UsdShadeMaterial GetUsdMaterial(const pxr::UsdSchemaBase& schemaObj)
    {
        ...
        pxr::UsdShadeMaterial material =
            material_binding.ComputeBoundMaterial(pxr::UsdShadeTokens->allPurpose, &binding_relationship);
        if (material) return material;
        // if the BindingRelation ship is invalid it means, that no material binding has been authored. This is ok and
        // will lead to a default material beeing used.
        if (!binding_relationship.IsValid()) return pxr::UsdShadeMaterial();

        // not being able to get the material binding could be, that the bound material is part of another usd layer
        // that is referenced via payload and the payload is not yet loaded. Check for this case. As this may be the
        // case along the path to the actual material on any parent level we walk the path up. even though there could
        // be technically multiple targets maintained for the material binding relationship we will only consider the
        // first one.
        ...
        for (const pxr::SdfPath& target: targets)
        {
            ...
            if (material_prim)
            {
                // now ensure the prim is loaded, this will also load any descendant prims. We assume, this load is not
                // heavy and will occur for the "materials in payload usd layers" use case
                material_prim.Load();
            ...
            for (int p = 0; p < pointCount; ++p)
            {
                ...
                if (!texcoords.empty())
                {
                    ...
                    else if (texcoordsInterpolation == pxr::UsdGeomTokens->faceVarying)
                    {
                        ...
                        int uvIndex =
                            texcoordIndices.empty() ? facePointOffset + p : texcoordIndices[facePointOffset + p];
                        uv = TypeConverter::toVector2(texcoords[uvIndex]);
                    }

                    // flip y value
                    // TODO: is there a reliable way to know when to flip and when not? Actually it seems always to be
                    // required
                    uv = FlipUvV(uv);
                }
                ...
                if (!joint_indices.empty())
                {
                    ...
                    for (int element = 0; element < joint_index_element_size; ++element)
                    {
                        ...
                        else
                        {
                            ...
                            // it has been ensured, that the skeleton has been converted already and contains a map from
                            // joint name to bone index
