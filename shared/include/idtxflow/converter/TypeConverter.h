 ...
 *
 * This includes basic math types like vectors, colors and transforms, but also more complex types like textures and
 * materials.
 */
...
class UsdTypeConverter
{
  public:
     ...
     * @return
     */
    inline static typename Types::Transform toTransform(const class pxr::GfMatrix4d& usdTransform,
                                                        const class pxr::TfToken& spineAxis = pxr::TfToken());
     ...
     * @return
     */
    inline static std::optional<typename Types::Texture> toTexture(const std::vector<uint8_t>& binaryData,
                                                                   const std::string& lower_extension,
...
class TargetMeshBuilder
{
  public:
    ...
    virtual void AddVertex(MeshDataType& mesh, const typename Types::Vector3& position,
                           const typename Types::Vector3& normal, const typename Types::Vector2& uv,
                           const std::vector<uint32_t>& boneIdx = {}, const std::vector<float>& boneWeight = {}) = 0;
