 ...
 * @brief Converter for USD animation data (primarily UsdGeomXformable and UsdSkelSkeleton) into a generic
 * AnimationDescription format that can be further processed into engine-specific animation assets.
 */
#include <map>
#include <optional>
#include <variant>
...
template <typename TargetEngine> struct AnimationTrackKey
{
...
template <typename TargetEngine> struct AnimationTrackDescription
{
    std::string Name;
...
template <typename TargetEngine> struct AnimationDescription
{
    float Length;
...
class UsdAnimationConverter
{
  public:
    ...
    std::optional<AnimationDescription<TargetEngine>> Convert(const pxr::UsdGeomXformable& usdXForm,
                                                              double usdTimeCodesPerSec)
    {
        ...
        for (double timecode: xform_timecodes)
        {
            ...
            if (!usdXForm.GetLocalTransformation(&tc_matrix, &resets, pxr::UsdTimeCode(timecode))) continue;
    ...
    std::optional<AnimationDescription<TargetEngine>> Convert(const pxr::UsdSkelRoot& usdSkelRoot,
