...
template <typename T>
concept TransformLike = requires(T m) { (std::is_class_v<T> && sizeof(T) >= sizeof(float) * 16); };
...
concept TargetEngineTypesLike = requires {
    ...
    { T::ResolveConvertedEntity(typename T::ConvertedEntityHandle{}) } -> std::same_as<typename T::ConvertedEntity*>;
...
template <typename TargetEngine> struct TargetEngineTypes;
