...
{
    return godot::Vector3(static_cast<float>(usd_vec[0]), static_cast<float>(usd_vec[1]),
...
{
    return godot::Vector4(static_cast<float>(usd_vec[0]), static_cast<float>(usd_vec[1]),
                          static_cast<float>(usd_vec[2]), static_cast<float>(usd_vec[3]));
};
...
inline godot::Quaternion UsdTypeConverter<types::TargetEngineGodot>::toQuaternion(const class pxr::GfQuatd& usd_quat)
{
    ...
    godot::Quaternion quaternion(static_cast<float>(vec[0]), static_cast<float>(vec[1]), static_cast<float>(vec[2]),
                                 static_cast<float>(w));
    return quaternion.normalized();
}

template <> inline godot::Color UsdTypeConverter<types::TargetEngineGodot>::toColor(const class pxr::GfVec4f& usd_color)
{
...
{
    godot::Basis basis(godot::Vector3(static_cast<float>(usd_transform[0][0]), static_cast<float>(usd_transform[0][1]),
                                      static_cast<float>(usd_transform[0][2])),
                       godot::Vector3(static_cast<float>(usd_transform[1][0]), static_cast<float>(usd_transform[1][1]),
                                      static_cast<float>(usd_transform[1][2])),
                       godot::Vector3(static_cast<float>(usd_transform[2][0]), static_cast<float>(usd_transform[2][1]),
                                      static_cast<float>(usd_transform[2][2])));

    godot::Vector3 origin(static_cast<float>(usd_transform[3][0]), static_cast<float>(usd_transform[3][1]),
                          static_cast<float>(usd_transform[3][2]));
    ...
    const std::vector<uint8_t>& binary_data, const std::string& lower_extension, const TexturePurpose purpose)
{
    ...
    {
        godot::print_verbose("Unable to convert texture with extension '", lower_extension.c_str(),
                             "' into Image. ErrorCode: ", static_cast<int>(img_error));
        return {};
    }
...
{
    if (token == pxr::TfToken("r")) return godot::BaseMaterial3D::TEXTURE_CHANNEL_RED;
    if (token == pxr::TfToken("g")) return godot::BaseMaterial3D::TEXTURE_CHANNEL_GREEN;
    if (token == pxr::TfToken("b")) return godot::BaseMaterial3D::TEXTURE_CHANNEL_BLUE;
    if (token == pxr::TfToken("a")) return godot::BaseMaterial3D::TEXTURE_CHANNEL_ALPHA;
    ...
    const pxr::UsdStageRefPtr& stage)
{
    ...
    for (const auto& channelData: material_description.channels)
    {
        ...
        case types::MaterialChannel::Diffuse: {
            if (auto texture = std::get_if<std::optional<types::TextureDescription<TextureType>>>(&channelValue);
                texture && texture->has_value())
            {
                ...
                if (auto godotTexture = UsdAssetConverter<types::TargetEngineGodot>::LoadTextureWithResolver(
                        textureDescription.filePath, stage, TexturePurpose::COLOR_TEXTURE))
                ...
                // even though multiple textures may have different scaling and offset authored in USD, Godot does only
                // support those values once for each material. So, the last channel providing those values will "win"
        ...
        case types::MaterialChannel::Metallic: {
                ...
                texture && texture->has_value())
            {
                const types::TextureDescription<TextureType>& textureDescription = texture->value();
                if (auto godotTexture = UsdAssetConverter<types::TargetEngineGodot>::LoadTextureWithResolver(
                        textureDescription.filePath, stage, TexturePurpose::METALLIC_TEXTURE))
                ...
                if (textureDescription.channel != pxr::TfToken())
                    standard_material->set_metallic_texture_channel(
                        get_material_texture_channel(textureDescription.channel));
        ...
        case types::MaterialChannel::Roughness: {
                ...
                texture && texture->has_value())
            {
                const types::TextureDescription<TextureType>& textureDescription = texture->value();
                if (auto godotTexture = UsdAssetConverter<types::TargetEngineGodot>::LoadTextureWithResolver(
                        textureDescription.filePath, stage, TexturePurpose::ROUGHNESS_TEXTURE))
                ...
                if (textureDescription.channel != pxr::TfToken())
                    standard_material->set_roughness_texture_channel(
                        get_material_texture_channel(textureDescription.channel));
        ...
        case types::MaterialChannel::Normal: {
                ...
                texture && texture->has_value())
            {
                const types::TextureDescription<TextureType>& textureDescription = texture->value();
                if (auto godotTexture = UsdAssetConverter<types::TargetEngineGodot>::LoadTextureWithResolver(
                        textureDescription.filePath, stage, TexturePurpose::NORMAL_TEXTURE))
        ...
        case types::MaterialChannel::AmbientOcclusion: {
                ...
                texture && texture->has_value())
            {
                ...
                if (auto godotTexture = UsdAssetConverter<types::TargetEngineGodot>::LoadTextureWithResolver(
                        textureDescription.filePath, stage, TexturePurpose::COLOR_TEXTURE))
                    standard_material->set_texture(godot::BaseMaterial3D::TEXTURE_AMBIENT_OCCLUSION,
                                                   godotTexture.value());
...
template <> class TargetMeshBuilder<idtxflow::types::TargetEngineGodot>
{
  public:
    using Types = idtxflow::types::TargetEngineTypes<idtxflow::types::TargetEngineGodot>;

    void AddVertex(Types::MeshData& meshData, const Types::Vector3& position, const Types::Vector3& normal,
                   const Types::Vector2& uv, const std::vector<uint32_t>& bones, const std::vector<float>& boneWeights)
    {
        ...
        if (!bones.empty())
        {
            ...
            // NaN. Leave such vertices at zero weight so the skeleton converter's
            // neutral-bone pass can detect and rebind them.
            if (weightSum > 0.0f)
            {
