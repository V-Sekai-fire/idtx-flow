...
class IDTXCollisionAPI : public UsdAPISchemaBase
{
  public:
    ...
    /// an invalid \p prim
    explicit IDTXCollisionAPI(const UsdPrim& prim = UsdPrim()) : UsdAPISchemaBase(prim)
    {
    }
    ...
    explicit IDTXCollisionAPI(const UsdSchemaBase& schemaObj) : UsdAPISchemaBase(schemaObj)
    {
    }
    ...
    ///
    IDTX_API
    static IDTXCollisionAPI Get(const UsdStagePtr& stage, const SdfPath& path);
    ...
    ///
    IDTX_API
    static IDTXCollisionAPI Apply(const UsdPrim& prim);
  ...
  public:
    ...
    /// the default for \p writeSparsely is \c false.
    IDTX_API
    UsdAttribute CreateCollisionInteractionTypesAttr(VtValue const& defaultValue = VtValue(),
