...
inline void AddAnimation(const converter::AnimationDescription<types::TargetEngineGodot>& animation, NodeType* node,
    ...
    if (!animation.Tracks.empty())
    {
        for (const auto& track: animation.Tracks)
        {
            ...
            case converter::TRACK_TRANSFORM: {
                ...
                {
                    godot::Transform3D animTransform =
                        std::get<types::TargetEngineTypes<types::TargetEngineGodot>::Transform>(key.Value);
                    godot_animation->position_track_insert_key(pos_track, key.Time, animTransform.origin);
                    godot_animation->rotation_track_insert_key(rot_track, key.Time,
                                                               animTransform.basis.get_rotation_quaternion());
            ...
            case converter::TRACK_POSITION: {
                ...
                for (const auto& key: track.Keys)
                {
                    godot::Vector3 anim_pos =
                        std::get<types::TargetEngineTypes<types::TargetEngineGodot>::Vector3>(key.Value);
            ...
            case converter::TRACK_ROTATION: {
                ...
                for (const auto& key: track.Keys)
                {
                    godot::Quaternion anim_rot =
                        std::get<types::TargetEngineTypes<types::TargetEngineGodot>::Quaternion>(key.Value);
...
template <>
inline IExecBridgeHandler* UsdStageConverter<types::TargetEngineGodot>::GetExecBridgeHandler(godot::Node3D* node)
{
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertXform(
    const godot::Transform3D& transform, const std::optional<AnimationDescription<types::TargetEngineGodot>>& animation)
{
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertCube(
    const godot::Transform3D& transform, const std::optional<AnimationDescription<types::TargetEngineGodot>>& animation,
    const std::optional<godot::Ref<godot::StandardMaterial3D>>& material, float cube_size,
    const pxr::VtArray<class pxr::GfVec4f>& display_colors, const class pxr::TfToken& color_interpolation)
{
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertCylinder(
    const godot::Transform3D& transform, const std::optional<AnimationDescription<types::TargetEngineGodot>>& animation,
    const std::optional<godot::Ref<godot::StandardMaterial3D>>& material, float cylinder_radius, float cylinder_height,
    const pxr::VtArray<class pxr::GfVec4f>& display_colors, const class pxr::TfToken& color_interpolation)
{
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertCone(
    const godot::Transform3D& transform, const std::optional<AnimationDescription<types::TargetEngineGodot>>& animation,
    const std::optional<godot::Ref<godot::StandardMaterial3D>>& material, float cone_radius, float cone_height,
    const pxr::VtArray<class pxr::GfVec4f>& display_colors, const class pxr::TfToken& color_interpolation)
{
    godot::Ref<godot::CylinderMesh> cylinder;
    cylinder.instantiate();
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertSphere(
    const godot::Transform3D& transform, const std::optional<AnimationDescription<types::TargetEngineGodot>>& animation,
    const std::optional<typename Types::Material>& material, float sphere_radius,
    const pxr::VtArray<class pxr::GfVec4f>& display_colors, const class pxr::TfToken& color_interpolation)
{
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertCollisionRoot(
    const godot::Transform3D& transform, const pxr::GfVec3f highlightColor, const std::string identifier,
    const bool enabled, const bool highlightable)
{
    UsdXformNode3D* converted_node = memnew(UsdXformNode3D);
    converted_node->set_transform(transform);
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertCollision(
    const godot::Transform3D& transform, const pxr::TfToken shape, const pxr::VtArray<pxr::TfToken> types,
    const pxr::GfVec3f axis, const double height, const double radius)
{
    ...
    if (height)
    {
        collisionNode->set_height(height);
    }
    if (radius)
    {
        collisionNode->set_radius(radius);
    }
    ...
    const pxr::VtArray<class pxr::GfVec4f>& display_colors, const class pxr::TfToken& color_interpolation)
{
    ...
    for (size_t MeshSection = 0; MeshSection < mesh_descriptions.size(); ++MeshSection)
    {
        ...
        godot::Ref<godot::StandardMaterial3D> standard_material;
        std::optional<godot::Ref<godot::StandardMaterial3D>> material =
            ConvertMaterial(mesh_descriptions[MeshSection].usdMaterial);
        if (material.has_value())
        {
            standard_material = material.value();
        }
    ...
    for (auto& skinTarget: skeleton_description.SkinTargets)
    {
        ...
        for (const auto& meshDescription: MeshDescriptions)
        {
            ...
            if (!meshDescription.meshData.Bones.is_empty())
            {
                ...
                {
                    const int stride =
                        (meshDescription.meshData.boneWeightCount == types::MeshData::BONEWEIGHT_COUNT_8) ? 8 : 4;
                    const int64_t vertexCount = weights.size() / stride;
                    for (int64_t vert = 0; vert < vertexCount; ++vert)
                    {
            ...
            mesh->add_surface_from_arrays(godot::Mesh::PRIMITIVE_TRIANGLES, mesh_arrays, godot::Array(),
                                          godot::Dictionary(), flags);

            godot::Ref<godot::StandardMaterial3D> standard_material;
            std::optional<godot::Ref<godot::StandardMaterial3D>> material =
                ConvertMaterial(meshDescription.usdMaterial);
            if (material.has_value())
            {
                standard_material = material.value();
            }
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertGprimPseudoInstance(
    const godot::Transform3D& transform, const pxr::UsdGeomGprim& usd_gprim, const pxr::SdfPath& usd_prototype_path)
{
    ...
    if (StagePrototypeMap.contains(usd_prototype_path))
    {
        ...
        godot::Transform3D global_instance_transform =
            TypeConverter::toTransform(xform_cache.GetLocalToWorldTransform(usd_gprim.GetPrim()));
        UsdMultiMeshInstanceNode3D* multimesh_node =
            static_cast<UsdMultiMeshInstanceNode3D*>(StagePrototypeMap.at(usd_prototype_path));
    ...
    pxr::UsdGeomXformCache xform_cache;
    godot::Transform3D global_transform =
        TypeConverter::toTransform(xform_cache.GetLocalToWorldTransform(usd_gprim.GetPrim()));
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertPrimWithPayload(
    const pxr::UsdPrim& usd_prim, const std::string& payload_uri, const godot::Transform3D& transform,
...
template <>
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertDatasource(
...
inline godot::Node3D* UsdStageConverter<types::TargetEngineGodot>::ConvertPrimPostProcess(
    const pxr::UsdPrim& usd_prim, godot::Node3D* converted_prim, godot::Node3D* converted_parent_prim)
{
    ...
    if (!usd_node)
        IDTX_LOG(IDTX_ERROR, "Unable to get the IUsdNode3D interface for this node: %s", usd_prim.GetName().GetText());
