...
namespace converter
{
        ...
        const SkeletonDescription<types::TargetEngineGodot>& skeleton_description)
    {
        ...
        for (auto& skinTarget: skeleton_description.SkinTargets)
        {
            ...
            if (animation.has_value())
            {
                const AnimationDescription<types::TargetEngineGodot>& animDesc = animation.value();
                bool hasBlendTracks = false;
                for (const AnimationTrackDescription<types::TargetEngineGodot>& t : animDesc.Tracks)
                {
                ...
                if (hasBlendTracks && blendShapeCount > 0)
                {
                    ...
                    for (const AnimationTrackDescription<types::TargetEngineGodot>& t : animDesc.Tracks)
                    {
                        ...
                        if (!grouped)
                        {
                            ...
                            for (const AnimationTrackKey<types::TargetEngineGodot>& key : t.Keys)
                            {
