...
{
    std::lock_guard<std::mutex> lock(state_mutex_);
    endpoint_uri_ = value.utf8().get_data();
}
...
{
    std::lock_guard<std::mutex> lock(state_mutex_);
    json_body_ = value.utf8().get_data();
}
...
void UsdRestDatasourceNode3D::_process(double delta)
{
    ...
    if (!http_client_.performRequest(args, [state, generation, url](const ix::HttpResponsePtr& response) {
            ...
            {
                IDTX_LOG(IDTX_WARN, "REST request failed for '{}' (HTTP {})", url, response ? response->statusCode : 0);
                return;
            }
            auto bridge = state->bridge.lock();
            if (!bridge) return;

            idtxflow::exec::StageAttributeUpdate update{state->prim_path, state->attribute_name,
                                                        pxr::VtValue(std::string(response->body)), generation, "rest"};
            if (!bridge->EnqueueAttributeUpdate(std::move(update)))
                IDTX_LOG(IDTX_DEBUG, "REST update rejected for '{}'", url);
        }))
    {
        state->request_in_flight.store(false);
        IDTX_LOG(IDTX_ERROR, "Unable to perform async request for '{}'", url);
    }
}
...
void UsdRestDatasourceNode3D::_bind_methods()
{
    ...
    ADD_PROPERTY(PropertyInfo(Variant::STRING, "endpoint_uri_", PROPERTY_HINT_NONE, "",
    ...
    ADD_PROPERTY(PropertyInfo(Variant::STRING, "query_", PROPERTY_HINT_NONE, "",
                              PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_READ_ONLY),
                 "set_query", "get_query");
    ...
    ADD_PROPERTY(PropertyInfo(Variant::STRING, "method_", PROPERTY_HINT_NONE, "",
                              PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_READ_ONLY),
    ...
    ADD_PROPERTY(PropertyInfo(Variant::STRING, "json_body_", PROPERTY_HINT_NONE, "",
                              PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_READ_ONLY),
                 "set_json_body", "get_json_body");

    ClassDB::bind_method(D_METHOD("set_refresh_interval", "p_interval"),
                         &UsdRestDatasourceNode3D::set_refresh_interval);
    ClassDB::bind_method(D_METHOD("get_refresh_interval"), &UsdRestDatasourceNode3D::get_refresh_interval);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "refresh_interval", PROPERTY_HINT_NONE, "",
                              PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_READ_ONLY),
